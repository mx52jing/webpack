...
然后打包我们的代码,可以看到打包后的代码成了84.6kb，这是因为打包将`lodash`的内容和我们`index.js`的内容打包到了一起，如果我们引用了其他很多第三方模块，都会打包到一个js文件，这样导致js文件过大，首屏加载时间肯定会很久，
用户体验也不好，我们希望将第三方模块的公共库代码和业务代码打包成不同的文件，这就要用到代码分割。
